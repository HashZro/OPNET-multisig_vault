/**
 * Deploy BetaToken (OP-20) to OPNet testnet
 *
 * Run: npm run deploy:beta
 *
 * Prerequisites:
 *  - .env with EC_PRIVATE_KEY (64 hex) and MLDSA_PRIVATE_KEY (5120 hex) set
 *  - build/BetaToken.wasm must exist (run: npm run build:beta)
...
console.log('=== BetaToken Deployment ===');
...
const wasmPath   = join(__dirname, '..', 'build', 'BetaToken.wasm');
...
console.log('Contract address:', result.contractAddress);
console.log('Contract pubkey :', result.contractPubKey);
console.log('\nSave the contract address — you will need it to interact with BetaToken.');
