/**
 * Deploy AlphaToken (OP-20) to OPNet testnet
 *
 * Run: npm run deploy:alpha
 *
 * Prerequisites:
 *  - .env with EC_PRIVATE_KEY (64 hex) and MLDSA_PRIVATE_KEY (5120 hex) set
 *  - build/AlphaToken.wasm must exist (run: npm run build:alpha)
...
//
// EC_PRIVATE_KEY  : 64 hex chars (32 bytes) — secp256k1 private key
// MLDSA_PRIVATE_KEY: 5120 hex chars (2560 bytes) — ML-DSA-44 private key
//
// Export both from OPWallet: Settings → Account → Export Private Keys.
// The MLDSA key shown in the UI is truncated; use the copy button to get the full key.
// ---------------------------------------------------------------------------
...
const wallet = Wallet.fromPrivateKeys(
    ...
    MLDSA_PK,
    NETWORK,
    MLDSASecurityLevel.LEVEL2,  // 44 = ML-DSA-44 = 128-bit post-quantum security
);

// Derive OPNet-format addresses (opt1p... / opt1q...) for UTXO lookup and `from` field
...
console.log('=== AlphaToken Deployment ===');
...
const wasmPath   = join(__dirname, '..', 'build', 'AlphaToken.wasm');
...
// ---------------------------------------------------------------------------
// Fetch epoch challenge (required for all OPNet transactions)
// ---------------------------------------------------------------------------
console.log('Fetching epoch challenge...');
const challenge = await rpcProvider.getChallenge();

// ---------------------------------------------------------------------------
// Sign both transactions (funding + deployment)
// ---------------------------------------------------------------------------
...
// (deployment tx spends the funding tx output)
// ---------------------------------------------------------------------------
...
console.log('Contract address:', result.contractAddress);
console.log('Contract pubkey :', result.contractPubKey);
console.log('\nSave the contract address — you will need it to interact with AlphaToken.');
